...
contract LiquidationOperator is IUniswapV2Callee {
    ...
    function operate() external {
        ...
        uniswapV2Pair_USDC_WETH.swap(debt_USDC, 0, address(this), "flashloan");
    ...
    function uniswapV2Call(
        ...
        // ดึง Reserves ของทั้ง 2 Pool มาเตรียมคำนวณ
        // (uint256 reserve_WBTC_Pool1, uint256 reserve_USDT_Pool1, ) = uniswapV2Pair_WBTC_USDT.getReserves();
        // (uint256 reserve_WBTC_Pool2, uint256 reserve_WETH_Pool2, ) = uniswapV2Pair_WBTC_WETH.getReserves();
        (uint256 reserve_USDC, uint256 reserve_WETH, ) = uniswapV2Pair_USDC_WETH.getReserves();
        
        //Check มีเหรียญใน pool เท่าไร
        // console.log("Reserve WBTC in Pool1:", reserve_WBTC_Pool1);
        // console.log("Reserve USDT in Pool1:", reserve_USDT_Pool1);


        // 2.1 liquidate the target user
        uint debtToCover = amount0; // ยอด USDT ที่ยืมมา
        ...
        // 2.2 คืนหนี้ Flash Swap ด้วย WBTC
        // คำนวณว่าต้องใช้ WBTC เท่าไหร่ไปคืนหนี้ USDT ที่ยืมมา
        uint repay_WETH = getAmountIn(debtToCover, reserve_WETH, reserve_USDC);
        WETH.transfer(address(uniswapV2Pair_USDC_WETH), repay_WETH); // โอนคืนตู้ 1

        // 2.3 เอากำไร WBTC ที่เหลือ ไป Swap เป็น WETH
        // uint profit_WBTC = collateral_WBTC - repay_WBTC; // นี่คือกำไรของเรา
        // WBTC.transfer(address(uniswapV2Pair_WBTC_WETH), profit_WBTC); // โอนไปตู้ 2
        
        // คำนวณว่าจะได้ WETH เท่าไหร่ แล้วสั่ง Swap
